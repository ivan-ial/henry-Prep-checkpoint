...
function stringMasLarga(strings) {
  ...
  var strMayor = strings[0];

  for (var i = 0; i < strings.length; i++){
    if (strings[i].length > strMayor.length){
      strMayor = strings[i];
    }
  }
...
function sumArray(array, n) {
  ...
  // [2,5,7,10,11,15,20] 
  //  i
  //    j (i+1 es para que no coincidan i y j)
  
  for (var i = 0; i < array.length; i++){
    for (var j = i + 1; j < array.length; j++){
      if (array[i] + array[j] === n) 
      return true;             // si el numero que esta apuntando i mas el que apunta j es = n
    }
  }
  return false;
};
  ...
  class Persona {
    ...
    getFriends() {
      ...
      // persona.getFriends() // retorna ['toni', 'Leo', 'Manu']
       ...
       return pluck(this.amigos,'nombre');
    }
    ...
    getPromedioEdad() {
      ...
      var total = this.amigos.reduce(function(acum, elemento){
